"""Exportação da tabela "Compras do período" (uma linha por parcela) em Excel (.xlsx) e PDF."""
...
CABECALHO = [
    "Compra", "Parcela", "Fornecedora", "Tipo de compra", "Data da compra",
...
def gerar_excel_compras(linhas, periodo_txt, visao_txt, totais):
    """linhas: lista de dicts (id, parcela, fornecedora, tipo_compra, data_aceite,
    ...
    for l in linhas:
        ws.append([
            l["id"], l["parcela"], l["fornecedora"], l["tipo_compra"], l["data_aceite"],
    ...
    for situacao, total in totais:
        ws.append([None, None, None, None, None, None, f"Total — {situacao}", None, float(round(total, 2))])
        r = ws.max_row
        ws.cell(r, 7).font = Font(bold=True)
        ws.cell(r, 9).font = Font(bold=True)
        ws.cell(r, 9).number_format = '"R$" #,##0.00'

    larguras = [9, 9, 30, 28, 15, 13, 12, 13, 14, 12]
...
def gerar_pdf_compras(linhas, periodo_txt, visao_txt, totais):
    ...
    for l in linhas:
        dados.append([
            p(f"#{l['id']}"), p(l["parcela"]), p(l["fornecedora"]), p(l["tipo_compra"]), p(fmt_data(l["data_aceite"])),
    ...
    larguras = [1.4, 1.4, 5.2, 4.0, 2.4, 2.4, 2.3, 2.4, 2.4, 2.1]
